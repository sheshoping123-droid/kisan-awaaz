...
class ImagePipeline:
    ...
    URDU_RETAKE_PHOTO_MESSAGES = {
        "blurry": (
            "معذرت، تصویر تھوڑی دھندلی ہے اور ہم صحیح تشخیص نہیں کر سکے۔ "
            "براہ کرم دن کی روشنی میں، کیمرہ متاثرہ پتے یا حصے کے قریب رکھ کر "
            "ایک صاف تصویر دوبارہ بھیجیں۔"
        ),
        "not_a_plant": (
            "معذرت، اس تصویر میں کوئی پودا واضح نظر نہیں آ رہا۔ براہ کرم متاثرہ "
            "فصل، پتے، یا پھل کی صاف تصویر بھیجیں۔"
        ),
        "too_far": (
            "تصویر بہت دور سے لی گئی ہے۔ براہ کرم متاثرہ پتے یا حصے کے قریب "
            "جا کر دوبارہ تصویر بھیجیں تاکہ علامات صاف نظر آئیں۔"
        ),
        "poor_lighting": (
            "روشنی کم ہونے کی وجہ سے تصویر صاف نہیں آئی۔ براہ کرم دن کی روشنی "
            "میں دوبارہ تصویر بھیجیں۔"
        ),
    }
    ...
    async def process(self, image_bytes: bytes) -> ImageAnalysisResult:
        ...
        logger.info(
            "ImagePipeline: crop=%s, condition=%s, disease=%s, confidence=%s, "
            "symptoms=%d, quality_issue=%s",
            vision_result.crop,
            vision_result.condition,
            vision_result.likely_disease or "-",
            vision_result.confidence,
            len(vision_result.symptoms),
            vision_result.image_quality_issue or "-",
        )

        if vision_result.image_quality_issue:
            retake_message = self.URDU_RETAKE_PHOTO_MESSAGES.get(
                vision_result.image_quality_issue,
                self.URDU_RETAKE_PHOTO_MESSAGES["blurry"],
            )
            logger.info(
                "ImagePipeline: image quality issue '%s', asking for retake instead of diagnosing",
                vision_result.image_quality_issue,
            )
    ...
    async def _generate_llm_response(
        ...
        disease_line = (
            f"Likely diagnosis: {vision.likely_disease}\n"
            if vision.likely_disease
            else ""
        )
        prompt = (
            ...
            f"If a specific disease/pest was identified above, name it "
            f"plainly in Urdu and give treatment steps specific to it "
            f"rather than generic advice."
        )
    ...
    def _build_query(self, result: VisionResult) -> str:
        ...
        parts = [result.crop]
        if result.likely_disease:
            parts.append(result.likely_disease)
        if result.symptoms:
    ...
    def _format_response(
        ...
        if vision.likely_disease:
            lines.append(f"ممکنہ بیماری/وجہ: {vision.likely_disease}")
        lines.append(f"علامات: {symptoms_text}")
